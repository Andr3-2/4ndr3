...
float contasC(int mes[],float VencimentoBase[], float SubsidioAlimentação[], float irs[], float total[], float SegurançaSocial[], float entidadePatronal[]){
    for(int i=0; i<12; i++){
        VencimentoBase[i] = mes[i] * 60;
        if(mes[i]>20){
            VencimentoBase[i] += VencimentoBase[i]*0.05;
        }if(mes[i]>17){
            VencimentoBase[i] += VencimentoBase[i]*0.02;
        }
        SubsidioAlimentação[i] = mes[i] * 7.5;
        total[i] += SubsidioAlimentação[i] += VencimentoBase[i];
        if(total[i]<1000){
            irs[i] = total[i]*0.1;
        }
...
float contasA(int mes[],float VencimentoBase[], float SubsidioAlimentação[], float irs[], float total[], float SegurançaSocial[], float entidadePatronal[]){
    for(int i=0; i<12; i++){
        VencimentoBase[i] = mes[i] * 60;
        if(mes[i]>20){
            VencimentoBase[i] += VencimentoBase[i]*0.05;
        ...
        SegurançaSocial[i] = total[i]*0.09;
        entidadePatronal[i] = total[i]*0.21;
    }
}
